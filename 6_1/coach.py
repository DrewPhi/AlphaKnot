...
class Coach:
    ...
    def execute_episode(self):
        """
        Plays one self-play episode using MCTS to determine moves.
        Returns training examples: (state, pi, value) for each move.
        """
        train_examples = []
        starting_player = 1 if config.knotter_first else -1
        game = KnotGame(self.game.initial_pd_code, starting_player)

        # Reset MCTS for each new episode
        ...
        while not game.game_over:
            ...
            # Record (state, pi, current_player)
    ...
    def get_policy(self, canonicalBoard, temp):
        """
        Turn MCTS visit counts into a probability distribution over actions.
        """
    ...
    def learn(self):
        """
        Main training loop. If require_perfect_random_win=True, training will
        continue beyond num_iterations until (after finishing the planned iterations)
        the champion wins 200/200 games against a random player in either role.
        We never stop earlier than num_iterations.
        """
        total_start_time = time.time()

        # Champion model directory
        ...
        i = 1
        champion_perfected = False  # becomes True if champion wins 200/200 in either role vs. random
        while True:
            ...
            for e in range(1, self.num_episodes + 1):
                ...
                print(f"Iteration {i}/{self.num_episodes}, Episode {e}/{self.num_episodes} "
            ...
            # Compare candidate vs. champion (head-to-head)
            self.compare_and_promote(i)

            # If required, check champion's performance vs random in both roles
            if config.require_perfect_random_win:
                ...
                champion_perfected = True  # so we can exit normally

            # Stopping condition: Only stop if we've completed at least num_iterations and champion is perfected in one role.
            if i >= self.num_iterations:
