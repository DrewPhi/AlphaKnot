...
def main():
    ...
    # Set config.max_crossings to match the model (e.g., 8 for the first 8 twist knots)
    config.max_crossings = 8  # Adjust as needed
    # Load the model
    pd_code = [
        [[1, 7, 2, 6, 0], [6, 1, 7, 2, 0]],
        [[3, 10, 4, 11, 0], [10, 4, 11, 3, 0]],
    ...
    # Pad the PD code
    pd_code_padded = pad_pd_code(pd_code.copy(), config.max_crossings)
    game = KnotGame([pd_code_padded])
