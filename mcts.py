...
class MCTS:
    ...
    def getActionProb(self, canonicalBoard,current_player, temp=1):
        ...
        for _ in range(self.args.numMCTSSims):
            self.search(state[0], state[1], is_root=True)

        s = self.game.stringRepresentation(state)
        counts = [self.Nsa.get((s, a), 0) for a in range(self.game.getActionSize())]

        # With a one-simulation smoke configuration, the first visit only
        # expands the root and produces no edge counts. Fall back to the
        # masked root prior instead of dividing by zero or choosing an invalid
        # action.
        if not any(counts):
            prior = np.asarray(self.Ps[s], dtype=float)
            if temp == 0:
                best_actions = np.flatnonzero(prior == prior.max())
                best_action = np.random.choice(best_actions)
                probs = [0] * len(counts)
                probs[best_action] = 1
                return probs
            return (prior / prior.sum()).tolist()
    ...
    def search(self, canonicalBoard, player, is_root=False):
        ...
        v = self.search(next_canonical, next_canonical_player, is_root=False)
