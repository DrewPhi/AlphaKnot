...
generalization. The variable-size script supports
\texttt{--split train --eval-split test}: training and checkpoint selection
use train metrics only, while frozen held-out exact tables grade each eval
epoch and the best checkpoint per knot (\texttt{HELDOUT\_SOLVED}). Held-out
exact labels grade but never train---reports must state this.
Self-play (G1.4) trains without exact targets and is compared
